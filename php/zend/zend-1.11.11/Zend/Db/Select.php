...
class Zend_Db_Select
{
    ...
    /**
     * Add a INDEX JOIN table and colums to the query.
     * A natural join assumes an equi-join across any column(s)
     * that appear with the same name in both tables.
     * Only natural inner joins are supported by this API,
     * even though SQL permits natural outer joins as well.
     ...
     * @param  array|string|Zend_Db_Expr $name The table name.
     * @param  array|string $cols The columns to select from the joined table.
     * @param  array|string $idx tThe 'use index' condition.
     * @param  string $schema The database name to specify, if any.
     * @return Zend_Db_Select This Zend_Db_Select object.
     * self::SQL_USE_INDEX,
     */
    ...
    public function joinIndex($name,$cond,$schema = null)
    {
        return $this->_join(self::INDEX_JOIN, $name, self::SQL_USE_INDEX,$cond,$col,$schema);
    }
