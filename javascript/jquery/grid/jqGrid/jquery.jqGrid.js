function jqGridInclude(pathFiles)
{
	var pathtojsfiles = pathFiles; // need to be ajusted
    // set include to false if you do not want some modules to be included
    var combineIntoOne = false;
    var combinedInclude = new Array();
    var combinedIncludeURL = "/biblioteca/javascript/jquery/grid/jqGrid/js/combine.php?type=javascript&files=";
    var minver = true;
    var modules = [
        ...
        if(modules[i].include === true) {
        	...
        	if (combineIntoOne !== true) {
        		if(jQuery.browser.safari || jQuery.browser.msie ) {
        			//faz o include dos arquivos js por jQuery.ajax
	...
	if ((combineIntoOne === true) && (combinedInclude.length>0) ) {
		...
		//alert(combinedIncludeURL+fileList);
